# -*- coding: utf-8 -*-
"""
Created on Wed Jun 14 19:44:31 2023
...
binary_otsu = np.uint8(255* (img_gray<Th_otsu))
cv2.imshow('', binary_otsu)
cv2.waitKey(0)
cv2.destroyAllWindows()


lunar = cv2.imread(r"C:\Users\JUANPABLOGASCA\Desktop\Deep_Vision\recursos\Lunar.jpg")
#Intensity Color (Average Channels - Gray's Scales)
img_gray2 = cv2.cvtColor(lunar, cv2.COLOR_BGR2GRAY) #BGT to GRAY

# OTSO METHOD
Th_otsu,_ = cv2.threshold(img_gray2,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)

binary_otsu = np.uint8(255* (img_gray2<Th_otsu))
cv2.imshow('', binary_otsu)
cv2.waitKey(0)
cv2.destroyAllWindows()
